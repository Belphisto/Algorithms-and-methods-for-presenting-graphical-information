...
namespace Coloring_closed_surfaces
{
    public class MyFigure
    {
        ...
        public bool Contains(PointF clickPoint)
        {

            int numPoints = CurrentPoints.Count;
            bool inside = false;

            for (int i = 0, j = numPoints - 1; i < numPoints; j = i++)
            {
                if (((CurrentPoints[i].Y > clickPoint.Y) != (CurrentPoints[j].Y > clickPoint.Y)) &&
                    (clickPoint.X < (CurrentPoints[j].X - CurrentPoints[i].X) * (clickPoint.Y - CurrentPoints[i].Y) / (CurrentPoints[j].Y - CurrentPoints[i].Y) + CurrentPoints[i].X))
                {
                    inside = !inside;
                }
            }
